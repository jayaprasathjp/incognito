...
1. User visits `/register` and fills the form:
   - **Alias** — public in-game name displayed to opponents
   ...
   - **Referral Code** (optional) — if someone referred them
   - **Agree to Rules** — mandatory checkbox accepting tournament rules

2. On submit → the server:
   - Checks no duplicate email or username exists
   - Creates user account with `role = 'player'`
...
### How a Player Joins (2-Step Process)

**Step 1 — Choose Session Preference:**
...
> ⚠️ **Important:** Session preference is a *preference*, NOT a guarantee. The system tries to match players in the same session, but may schedule outside the preference based on bracket constraints.

**Step 2 — Payment:**

Player reviews the summary (tournament name, chosen session, fee amount) and clicks **"PAY ₦X"**.

### What Happens Behind the Scenes (Payment)

1. Frontend sends `POST /payment/initialize` with tournament ID and session preference
